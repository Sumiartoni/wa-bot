...
async function loadSettings(){try{const s=await api('/api/settings');document.getElementById('sModel').value=s.ai_model||'';document.getElementById('sPrompt').value=s.ai_system_prompt||'';document.getElementById('sQrisUrl').value=s.qris_url||'';document.getElementById('sGroqKey').value=s.groq_api_key||'';document.getElementById('sOpenrouterKey').value=s.openrouter_api_key||'';document.getElementById('sChatgptToken').value=s.chatgpt_access_token||'';aiProvider=s.ai_provider||'groq';updateProviderUI();updateAIToggle(s.global_ai_enabled==='true');renderLabels()}catch(e){}}
