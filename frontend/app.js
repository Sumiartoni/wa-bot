...
function updateProviderUI(){['pGroq','pOpenai','pChatgpt'].forEach(id=>{const el=document.getElementById(id);if(!el)return;const k=id.replace('p','').toLowerCase();el.style.borderColor=aiProvider===k?'var(--primary)':'var(--border)';el.style.background=aiProvider===k?'rgba(99,102,241,.08)':''})}
