...
function toast(m,t='info'){const e=document.getElementById('toast');e.textContent=m;e.style.color=t==='success'?'rgb(var(--success))':'rgb(var(--danger))';e.classList.remove('hidden');setTimeout(()=>e.classList.add('hidden'),3000)}
...
function sc(l,v,ic,cls){return`<div class="stat-card ${cls}"><div style="display:flex;align-items:center;gap:8px;margin-bottom:8px"><i data-lucide="${ic}" style="width:16px;height:16px;opacity:.6"></i><span style="font-size:12px;color:rgb(var(--text2))">${l}</span></div><p style="font-size:26px;font-weight:700">${v}</p></div>`}
async function loadAgentStats(){try{const s=await api('/api/stats/agents');document.getElementById('agentLeaderboard').innerHTML=s.length?s.map(a=>`<div style="display:flex;align-items:center;justify-content:space-between;padding:10px 0;border-bottom:1px solid rgba(var(--border),.06)"><div style="display:flex;align-items:center;gap:10px"><div class="avatar avatar-sm" style="background:${a.avatar_color}">${a.name[0]}</div><span style="font-size:13px;font-weight:500">${a.name}</span></div><span style="font-size:12px;color:rgb(var(--text3))">${a.total_replies} replies &middot; ${a.assigned_chats} chats</span></div>`).join(''):'<p style="font-size:13px;color:rgb(var(--text3))">Belum ada data</p>';icons()}catch(e){}}
...
const stCls=c.chat_status==='open'?'badge-open':c.chat_status==='in_progress'?'badge-progress':'badge-resolved';
...
<p style="font-size:11px;color:rgb(var(--text3));overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin-top:2px">${c.last_message||'...'}</p></div></div>`}).join('');icons()}
...
area.innerHTML=`<div class="surface" style="padding:14px 20px;border-bottom:1px solid rgba(var(--border),.06);display:flex;align-items:center;justify-content:space-between">
...
<div><p style="font-size:14px;font-weight:600">${user.name||user.phone}</p><p style="font-size:11px;color:rgb(var(--text3))">${user.phone||''} <span class="badge ${stCls}" style="margin-left:4px">${user.chat_status}</span></p></div></div>
...
<div style="padding:14px 20px;border-top:1px solid rgba(var(--border),.06)"><div id="qrSuggest" class="hidden" style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:8px"></div>
...
document.getElementById('rightPanel').innerHTML=`<h3 style="font-size:13px;font-weight:700;margin-bottom:12px;display:flex;align-items:center;gap:6px"><i data-lucide="user" style="width:14px;height:14px;color:rgb(var(--primary))"></i>Customer Info</h3>
<div style="font-size:12px;display:grid;gap:8px">
<div><span style="color:rgb(var(--text3))">Nama</span><p style="font-weight:500">${user.name||'-'}</p></div>
<div><span style="color:rgb(var(--text3))">Phone</span><p style="font-weight:500">${user.phone}</p></div>
<div><span style="color:rgb(var(--text3))">Total Pesan</span><p style="font-weight:500">${user.total_messages}</p></div>
<div><span style="color:rgb(var(--text3))">Agent</span><p style="font-weight:500">${user.agent_name||'Belum assign'}</p></div>
<div><span style="color:rgb(var(--text3))">Priority</span><select onchange="setPriority('${jid}',this.value)" style="font-size:11px;padding:4px 8px;width:auto;margin-top:2px">${['low','normal','high','urgent'].map(p=>`<option value="${p}" ${user.priority===p?'selected':''}>${p}</option>`).join('')}</select></div>
<div><span style="color:rgb(var(--text3))">AI</span><button onclick="toggleUserAI('${jid}',${user.ai_enabled?0:1})" class="toggle ${user.ai_enabled?'on':'off'}" style="margin-top:4px"><span class="dot"></span></button></div></div>
<div class="divider" style="margin:14px 0"></div>
<h3 style="font-size:13px;font-weight:700;margin-bottom:8px;display:flex;align-items:center;gap:6px"><i data-lucide="tag" style="width:14px;height:14px;color:rgb(var(--accent))"></i>Labels</h3>
...
<div class="divider" style="margin:14px 0"></div>
<h3 style="font-size:13px;font-weight:700;margin-bottom:8px;display:flex;align-items:center;gap:6px"><i data-lucide="sticky-note" style="width:14px;height:14px;color:rgb(var(--warning))"></i>Notes</h3>
${notes.map(n=>`<div class="note-card" style="margin-bottom:8px"><p style="font-size:12px">${esc(n.content)}</p><p style="font-size:10px;color:rgb(var(--text3));margin-top:4px">${n.agent_name} <span style="cursor:pointer;color:rgb(var(--danger));margin-left:6px" onclick="deleteNote(${n.id},'${jid}')">hapus</span></p></div>`).join('')}
...
function renderQR(){document.getElementById('qrList').innerHTML=quickReplies.map(r=>`<div class="card" style="padding:16px;display:flex;justify-content:space-between;align-items:start"><div><p style="font-size:13px;font-weight:600">${r.shortcut} <span style="color:rgb(var(--text3));font-weight:400">— ${r.title}</span></p><p style="font-size:12px;color:rgb(var(--text2));margin-top:4px">${r.content}</p></div><button onclick="deleteQR(${r.id})" class="btn btn-danger" style="padding:6px 10px"><i data-lucide="trash-2" style="width:14px;height:14px"></i></button></div>`).join('')||'<p style="color:rgb(var(--text3));font-size:13px">Belum ada quick reply</p>';icons()}
...
function updateProviderUI(){['pGroq','pOpenai','pChatgpt'].forEach(id=>{const el=document.getElementById(id);if(!el)return;const k=id.replace('p','').toLowerCase();el.style.borderColor=aiProvider===k?'rgb(var(--primary))':'rgba(var(--border),.06)';el.style.background=aiProvider===k?'rgba(var(--primary),.08)':''})}
...
async function saveSettings(){const d={ai_provider:aiProvider,ai_model:document.getElementById('sModel').value,ai_system_prompt:document.getElementById('sPrompt').value};const g=document.getElementById('sGroqKey').value,o=document.getElementById('sOpenaiKey').value,c=document.getElementById('sChatgptToken').value;if(g)d.groq_api_key=g;if(o)d.openai_api_key=o;if(c)d.chatgpt_access_token=c;await api('/api/settings',{method:'PUT',body:JSON.stringify(d)});toast('Settings disimpan!','success')}
...
el.innerHTML=`<span style="width:7px;height:7px;border-radius:50%;background:rgb(${colors[s.status]||colors.disconnected});${s.status==='connecting'?'animation:pulseDot 2s infinite':''}"></span><span style="font-size:11px;color:rgb(var(--text3))">${labels[s.status]||s.status}</span>`;
